      ...
      loader.load(
        ...
        async (gltf) => {
          ...
          try {
            ...
            console.error('Error in model setup:', e);
            // Ensure loading finishes even on error
            if (onReady) onReady();
          }
        },
        (xhr) => {
          if (onProgress) {
             ...
             if (total > 0) {
               ...
               // Fallback if total is not provided (chunked encoding)
               // Just animate the loader a bit or keep it at 0.5
               onProgress(0.5); 
             }
          }
        },
        (err) => {
          ...
          // Ensure loading screen is removed even if model fails
          if (onReady) onReady();
        }
      );
